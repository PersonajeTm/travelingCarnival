...
function crosshair_update(crosshair_system, particle_system, target_system, score_system, replay_system, sprite_system, tick) {
    let copy_crosshair = {
...
function target_update(target_system, sprite_system) {
    for (let i = target_system.objects.length - 1; i >= 0; i--) {
        ...
        if (target.hit ||
            ...
            target.y + Math.abs(target.sprite.h) * target.sprite.tex.height < 0 ||
            target.y + Math.abs(target.sprite.h) * target.sprite.tex.height > HEIGHT) {
...
function anim_countdown(menu_system, score_system, tick, renderer) {
    ...
    renderer._ctx.fillStyle = "#060099";
...
function game_play(game) {
    ...
    game.score_system.total_score = 5;
...
function game_replay(game) {
    game.score_system.total_score = 10;
    game.replay_system.event_ptr = 0;
    game_reset(game);
...
function game_main_update(game) {
    crosshair_update(game.crosshair_system, game.particle_system, game.target_system, game.score_system, game.replay_system, game.sprite_system, game.tick);
